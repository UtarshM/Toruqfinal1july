<?php  
	...
	if($date_type==1){
	  ...
	  if ( $_SESSION[ 'adm_type' ] == 0 ) { // Admin 
		// Check Module Rights
		$queryCollection = "SELECT * FROM renewal_detail td,  status_detail st  where td.ren_status!=3 and td.ren_action IN (1,2) ".$dtws_qu." and st.status_id=td.ren_status  and td.branch_id=".$_SESSION[ 'adm_branch' ]." ORDER BY td.ren_id";
	  } else {
		// Check Module Rights
		$queryCollection = "SELECT * FROM renewal_detail td,  status_detail st  where td.ren_status!=3 and td.ren_action IN (1,2) ".$dtws_qu." and st.status_id=td.ren_status  and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.ren_adm_id=".$_SESSION[ 'adm_id' ]." ORDER BY td.ren_id";
	  }
	...
	if($total_records_rep<=0){
		header( 'Location: renewal_view.php?msg=empty&strdate='.$strdate3.'&enddate='.$enddate3.'&date_type='.$date_type.'' );
	}
?>
<?php
$date = time();
header("Content-type: application/vnd-ms-excel");
	header("Content-Disposition: attachment; filename=Renewal-".$fl_nam."ReportDateWise-$strdate22.xls");
?>
<table border="1" style="text-align: left;">
  <tr style="text-align: left; background:#000; color: #fff;">
    ...
    <th style="text-align: left;">Net Premium</th>
    <th style="text-align: left;">Total Premium</th>
  ...
  <?php
	...
	while ($collectionData = $result_collaction->fetch_object()) {
			...
			if($date_type==1){
				$datend = new DateTime( $collectionData->ren_insurance_date );
				$dt_dis = $datend->format( 'd-m-Y' );
			} else if($date_type==2){
			$datend = new DateTime( $collectionData->ren_cf_date );
			$dt_dis = $datend->format( 'd-m-Y' );
			} else if($date_type==3){
			$datend = new DateTime( $collectionData->ren_reg_date );
				$dt_dis = $datend->format( 'd-m-Y' );
			} else if($date_type==4){
			$datend = new DateTime( $collectionData->ren_permit_date );
			$dt_dis = $datend->format( 'd-m-Y' );
			} else if($date_type==5){
			$datend = new DateTime( $collectionData->ren_nat_permit_date );
				$dt_dis = $datend->format( 'd-m-Y' );
			} else if($date_type==6){
			$datend = new DateTime( $collectionData->ren_tax_date );
			$dt_dis = $datend->format( 'd-m-Y' );
			} else {
			$datend = new DateTime( $collectionData->ren_date );
			$dt_dis = $datend->format( 'd-m-Y' );
			}

			$query_asign_detail = "SELECT * FROM admin_login ld where ld.adm_id=".$collectionData->ren_adm_id;
			$asign_query = $con->query( $query_asign_detail );
			$row_asign = $asign_query->fetch_object();
			$asign_name = $row_asign->adm_username;

			if($collectionData->ren_action==1) {
				$st_name = "Pending";
			  } elseif($collectionData->ren_action==2) {
				$st_name = "Rejected - ";
				...
				$st_res = $rejres_name;
			  } elseif($collectionData->ren_action==3) {
				$st_name = "Completed";
			  }
 			echo '<tr style="background:'.$tr_col.'">';
			echo '<td>'.$collectionData->ren_series.'</td>';	
			echo '<td>'.$dt_dis.'</td>';	
			echo '<td>'.$collectionData->ren_reg_no.'</td>';	
			echo '<td>'.$collectionData->ren_name.'</td>';	
			echo '<td>'.$collectionData->ren_contact.'</td>';	
			echo '<td>'.$collectionData->ren_vmodel.'</td>';	
			echo '<td>'.$collectionData->ren_netprem.'</td>';	
			echo '<td>'.$collectionData->ren_totprem.'</td>';	
