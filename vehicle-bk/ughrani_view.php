...
// and Date(td.ugh_date) >= '$to_da' and Date(td.ugh_date) <= '$la_da'
// and Date(td.ugh_date) >= '$to_da' and Date(td.ugh_date) <= '$la_da'
// and Date(td.ugh_date) >= '$to_da' and Date(td.ugh_date) <= '$la_da'
// and Date(td.ugh_date) >= '$to_da' and Date(td.ugh_date) <= '$la_da'
...
$enddate = date("Y-m-d", strtotime($enddate));
  ...
  $query_expe_tot = "SELECT * FROM ughrani_detail td where  td.ugh_status=1 ".$aol_tp_qu." and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.ugh_status IN (1,2) and Date(td.ugh_date) >= '$strdate' and Date(td.ugh_date) <= '$enddate' ORDER BY td.ugh_id";
  $result_tot = $con->query( $query_expe_tot );
  $total_records = $result_tot->num_rows;

  $query_expe = "SELECT * FROM ughrani_detail td  where td.ugh_status=1 ".$aol_tp_qu." and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.ugh_status IN (1,2) and Date(td.ugh_date) >= '$strdate' and Date(td.ugh_date) <= '$enddate'  ORDER BY td.ugh_id";

  $query_income = "SELECT SUM(ugh_amount) as total_income FROM ughrani_detail td where  td.ugh_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and Date(td.ugh_date) >= '$strdate' and Date(td.ugh_date) <= '$enddate'";
	$result_income = $con->query( $query_income );
	$row_income = $result_income->fetch_object();
	$final_income = $row_income->total_income;

	$query_expense = "SELECT SUM(ugh_amount) as total_expense FROM ughrani_detail td where td.ugh_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and Date(td.ugh_date) >= '$strdate' and Date(td.ugh_date) <= '$enddate'";
...
$query_expe_tot = "SELECT * FROM ughrani_detail td where  td.ugh_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.ugh_status IN (1,2)  ORDER BY td.ugh_id";
$result_tot = $con->query( $query_expe_tot );
$total_records = $result_tot->num_rows;
// echo $total_records; exit;

$query_expe = "SELECT * FROM ughrani_detail td  where td.ugh_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.ugh_status IN (1,2)   ORDER BY td.ugh_id";

  $query_income = "SELECT SUM(ugh_amount) as total_income FROM ughrani_detail td where td.ugh_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." ";
	$result_income = $con->query( $query_income );
	$row_income = $result_income->fetch_object();
	$final_income = $row_income->total_income;

	$query_expense = "SELECT SUM(ugh_amount) as total_expense FROM ughrani_detail td where td.ugh_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." ";
	$result_expense = $con->query( $query_expense );
	$row_expense = $result_expense->fetch_object();
	$final_expense = $row_expense->total_expense;
...
if ($_POST["submit_report"] != "") {
  ...
  $enddate = $_POST['enddate'];
   $strdate = date("Y-m-d", strtotime($strdate));
  $enddate = date("Y-m-d", strtotime($enddate));
	header( 'Location: ughrani_export.php?strdate=' . $strdate . '&enddate=' . $enddate.'' );
}
...
<title>Ughrani List   - <?php echo $meta_title; ?></title>
   ...
   function deleterec(id)
	{
		...
		{
			window.location="ughrani_delete.php?ugh_id="+id;
		}
	} 
	</script> 
...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-table"></i> Ughrani List   </h2>
      <div class="breadcrumb-wrapper"> <span class="label">You are here:</span>
        <ol class="breadcrumb">
          <li><a style="color:#1C1B17;" href="#">Dashboard</a></li>
          <li class="active">Ughrani List  </li>
        </ol>
      </div>
    </div>
      ...
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          if ( isset( $_GET[ 'flag' ] ) ) {
            ?>
          <?php if($_GET['flag']==1) {?>
          <p class="mb20" style="color:green">Ughrani details added successfully.</p>
          <?php } else if($_GET['flag']==2) {?>
          <p class="mb20" style="color:green">Ughrani details updated successfully.</p>
          <?php } else if($_GET['flag']==3) {?>
          <p class="mb20" style="color:green">Ughrani details deleted successfully.</p>
          <?php } ?>
          <?php } ?>
              ...
              <tbody>
                ...
                <tr class="odd gradeX">
                  ...
                  if($row_state->ugh_action==2) { echo "Completed"; }
                   ?></td>
                   <td   width="17%"><code> <a title="Edit" style="color:green;" href="ughrani_edit.php?ugh_id=<?php echo $row_state->ugh_id?>"><i class="fa fa-pen"></i></a> | <a title="Delete" style="color:red;" href="javascript:deleterec(<?php echo $row_state->ugh_id?>)"><i class="fa fa-trash"></i></a> <code></td>
                </tr>
                <?php }} ?>
              </tbody>
              <tfoot>
