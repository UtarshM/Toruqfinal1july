...
function addSociete2( $adresse2 )
{
	...
	$this->SetXY( $x1, $y1 + 4 );
	$this->SetFont('Arial','',10);
	$length = $this->GetStringWidth( $adresse2 );
	//Coordonn�es de la soci�t�
	$lignes = $this->sizeOfText( $adresse2, $length) ;
	$this->MultiCell($length, 6, $adresse2);
}
...
function addDate( $date )
{
	...
	$this->SetFont( "Arial", "", 10);
	$this->Cell(10,5, "Date : ".$date, 0, 0, "C");
...
function addClientAdresse( $adresse )
{
	...
	$y1     = 12;
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 100, 6, $adresse);
}
function addClientAdresse2( $adresse2 )
{
	$r1     = $this->w - 280;
	$r2     = $r1 + 68;
	$y1     = 115;
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 100, 6, $adresse2);
}
function addClientAdresse6( $adresse2 )
{
	$r1     = $this->w - 150;
	$r2     = $r1 + 68;
	$y1     = 115;
...
function addClientAdresse3( $adresse3 )
{
	$r1     = $this->w - 160;
	$r2     = $r1 + 68;
	$y1     = 270;
	$this->SetXY( $r1, $y1);
	$this->MultiCell( 200, 5, $adresse3);
}
function addClientAdresse4( $adresse4 )
{
	$r1     = $this->w - 200;
	$r2     = $r1 + 68;
	$y1     = 105;
...
function addReglement( $mode )
{
	...
	$this->SetFont( "Arial", "B", 10);
	$this->Cell(10,4, "Total Amount", 0, 0, "C");
...
function addNumTVA($tva)
{
	...
	$this->Cell(10, 4, "SGST", '', '', "C");
	$this->SetFont( "Arial", "", 10);
	$this->SetXY( $r1 + 10 , $y1+5 );
	$this->Cell(10, 5, $tva, '', '', "C");
}
function addNumTVA2($tva2)
{
	$this->SetFont( "Arial", "B", 10);
	$r1  = $this->w - 110;
	$r2  = $r1 + 30;
	...
	$this->Line( $r1, $mid, $r2, $mid);
	$this->SetXY( $r1 + 10 , $y1+1 );
	$this->Cell(10, 4, "Total Tax", '', '', "C");
	$this->SetFont( "Arial", "", 10);
	$this->SetXY( $r1 + 10 , $y1+5 );
	$this->Cell(10, 5, $tva2, '', '', "C");
}
...
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth( "Amount In Words: " . $remarque );
	$r1  = 10;
	$r2  = $r1 + $length;
	$y1  = $this->h - 45.5;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4, "Amount In Words:" . $remarque);
}
...
function temporaire( $texte )
{
	...
	$this->Rotate(25,50,70);
	$this->Text(20,100,$texte);
	$this->Rotate(0);
