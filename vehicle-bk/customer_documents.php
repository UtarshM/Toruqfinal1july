<?php
	...
	// Check Module Rights
	$cus_id = $_GET['cus_id'];
	$query_file = "SELECT * FROM customer_detail ld where ld.cus_status!=3  and ld.cus_id=".$cus_id;
	$result_query = $con->query($query_file);
	$row_file=$result_query->fetch_object();
	
	// echo "Test"; exit;
 	$total_document_detail = "SELECT * FROM status_detail st, document_detail sd  where sd.document_status=st.status_id and sd.document_status!='3' and sd.cus_id=".$cus_id." ORDER BY sd.document_id";
	$result_tot = $con->query($total_document_detail);
	$total_rec_gal=$result_tot->num_rows;
	// echo $total_records; exit;
	$query_document_rec = "SELECT * FROM status_detail st, document_detail sd  where sd.document_status=st.status_id and sd.document_status!='3' and sd.cus_id=".$cus_id."  ORDER BY sd.document_id";
	if(isset($_POST["submit"]))
	{
		...
		{	
				...
				$valid_exts_img = array('jpg','png','jpeg','JPG','PNG','JPEG','pdf','PDF');
					if (in_array($file_ext_img, $valid_exts_img)) { 
						...
						{				
							...
							$sql_document_ins = "INSERT INTO document_detail (document_title, document_image, cus_id,  added_by, updated_by, document_status, added_date, updated_date) VALUES ('".$document_title."','".$benner."','".$cus_id."','".$added_by."','".$updated_by."','1','".$added_date."','".$updated_date."')";
			...
			// echo " IN Valid File";
			header("Location: customer_documents.php?cus_id=".$_GET["cus_id"]."&msg=erimgext");  exit;
    ...
    ////// IMAGE UPLOAD
		...
		// 	}
 		// }
 		header('Location: customer_documents.php?flag=1&cus_id='.$cus_id);
	}
...
<title>Documents -
<?php  echo  $row_file->cus_name."(".$row_file->cus_code_no.")"; ?>
    ...
    <div class="pageheader">
      ...
      <div class="breadcrumb-wrapper"> <span class="label">You are here:</span>
        <ol class="breadcrumb">
          ...
          <li><a href="customer_view.php">Our Customer List </a></li>
		  ...
		  ?>
              ...
              <div class="panel-footer">
                ...
                <div class="col-sm-2" style="border: 1px solid #ddd;">
                  ...
                  <a style="color:red" onClick="return confirm('Are you sure want to delete?')"  href="customer_document_delete.php?document_id=<?php echo  $gallery->document_id; ?>&cus_id=<?php echo $cus_id; ?> " class="link" style="font-size:12px"><i class="fa fa-trash"></i></a> </div>
                <?php } else { ?>
                <div class="col-sm-2" style="border: 1px solid #ddd;"> <a href="img/document_files/<?php  echo $gallery->document_image; ?>"  data-rel="prettyPhoto"> <img style="margin-bottom: 5px;" src="img/document_files/<?php  echo $gallery->document_image; ?>" class="img-responsive"></a>
                  ...
                  <br>
                  <a style="color:red" onClick="return confirm('Are you sure want to delete?')"  href="customer_document_delete.php?document_id=<?php echo  $gallery->document_id; ?>&cus_id=<?php echo $cus_id; ?> " class="link" style="font-size:12px"><i class="fa fa-trash"></i></a> <br>
                  <br>
                </div>
                <?php } ?>
                <?php
