...
// and Date(td.chq_date) >= '$to_da' and Date(td.chq_date) <= '$la_da'
// and Date(td.chq_date) >= '$to_da' and Date(td.chq_date) <= '$la_da'
// and Date(td.chq_date) >= '$to_da' and Date(td.chq_date) <= '$la_da'
// and Date(td.chq_date) >= '$to_da' and Date(td.chq_date) <= '$la_da'
...
$enddate = date("Y-m-d", strtotime($enddate));
  ...
  $query_expe_tot = "SELECT * FROM cheque_detail td where  td.chq_status=1 ".$aol_tp_qu." and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.chq_status IN (1,2) and Date(td.chq_date) >= '$strdate' and Date(td.chq_date) <= '$enddate' ORDER BY td.chq_id";
  $result_tot = $con->query( $query_expe_tot );
  $total_records = $result_tot->num_rows;

  $query_expe = "SELECT * FROM cheque_detail td  where td.chq_status=1 ".$aol_tp_qu." and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.chq_status IN (1,2) and Date(td.chq_date) >= '$strdate' and Date(td.chq_date) <= '$enddate'  ORDER BY td.chq_id";
...
$query_expe_tot = "SELECT * FROM cheque_detail td where  td.chq_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.chq_status IN (1,2)  ORDER BY td.chq_id";
$result_tot = $con->query( $query_expe_tot );
$total_records = $result_tot->num_rows;
// echo $total_records; exit;

$query_expe = "SELECT * FROM cheque_detail td  where td.chq_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.chq_status IN (1,2)   ORDER BY td.chq_id";

  $query_income = "SELECT SUM(chq_amount) as total_income FROM cheque_detail td where td.chq_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." ";
	$result_income = $con->query( $query_income );
	$row_income = $result_income->fetch_object();
	$final_income = $row_income->total_income;

	$query_expense = "SELECT SUM(chq_amount) as total_expense FROM cheque_detail td where td.chq_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." ";
	$result_expense = $con->query( $query_expense );
	$row_expense = $result_expense->fetch_object();
	$final_expense = $row_expense->total_expense;
...
if ($_POST["submit_report"] != "") {
  ...
  $enddate = $_POST['enddate'];
   $strdate = date("Y-m-d", strtotime($strdate));
  $enddate = date("Y-m-d", strtotime($enddate));
	header( 'Location: cheque_export.php?strdate=' . $strdate . '&enddate=' . $enddate.'' );
}
...
<title>Cheque List   - <?php echo $meta_title; ?></title>
   ...
   function deleterec(id)
	{
		...
		{
			window.location="cheque_delete.php?chq_id="+id;
		}
	} 
	</script> 
...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-table"></i> Cheque List   </h2>
      ...
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          if ( isset( $_GET[ 'flag' ] ) ) {
            ?>
          <?php if($_GET['flag']==1) {?>
          <p class="mb20" style="color:green">Cheque details added successfully.</p>
          <?php } else if($_GET['flag']==2) {?>
          <p class="mb20" style="color:green">Cheque details updated successfully.</p>
          <?php } else if($_GET['flag']==3) {?>
          <p class="mb20" style="color:green">Cheque details deleted successfully.</p>
          <?php } ?>
          <?php } ?>
            ...
            <table id="example" class="table table-success mb30 table-hover table-bordered display" style="color:#000;" >
              ...
              <tbody>
                ...
                while ( $row_state = $result->fetch_object() ) {
                  ?>
                <tr class="odd gradeX">
                  <td ><?php echo $row_state->chq_id;?></td>
                  <td ><?php echo $row_state->chq_code_no;?></td>
                  <td ><a title="Edit" style="color:green;" href="cheque_edit.php?chq_id=<?php echo $row_state->chq_id?>"><?php echo $row_state->chq_no;?></a></td>
                  <td ><?php echo $row_state->chq_regno;?></td>

                  <td ><?php
                  $datend = new DateTime( $row_state->chq_date );
                  echo $datend->format( 'D, d-m-Y' );
                  ?></td>
                   <td><?php echo "&#8377;".number_format($row_state->chq_amount,2);?></td>
                  <td><?php 
                  if($row_state->chq_action==1) { echo "Pending"; }
                  if($row_state->chq_action==2) { echo "Cleared"; }
                  if($row_state->chq_action==3) { echo "Bounce"; }
                  ?></td>
                   <td   width="17%"><code> <a title="Edit" style="color:green;" href="cheque_edit.php?chq_id=<?php echo $row_state->chq_id?>"><i class="fa fa-pen"></i></a> | <a title="Delete" style="color:red;" href="javascript:deleterec(<?php echo $row_state->chq_id?>)"><i class="fa fa-trash"></i></a> <code></td>
                </tr>
                <?php }} ?>
              </tbody>
              <tfoot>
