...
if ( isset( $_POST[ "submit" ] ) ) {
  ...
  $query_ctg_dup = "SELECT * FROM category_detail where ctg_id!='" . $ctg_id . "' and ctg_name='" . $ctg_name . "' and ctg_status='1'";
  ...
  if ( $total_records_dup >= 1 ) {
    ...
    $sql_category_updt = "UPDATE category_detail SET ctg_name='" . $ctg_name . "', updated_by='" . $updated_by . "', ctg_status='" . $ctg_status . "',  ctg_updated='" . $ctg_updated . "' WHERE ctg_id=" . $ctg_id;
    if ( $con->query( $sql_category_updt ) === TRUE ) {
      header( 'Location: category_view.php?flag=2' );
    } else {
      header( 'Location: category_edit.php?ctg_id=' . $ctg_id );
    }
  }
}
$query_state_detail = "SELECT * FROM category_detail ld where ld.ctg_id=" . $ctg_id;
...
<title>Category Edit -<?php echo " ".$project_title." "; ?></title>
...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-pen"></i> Category Edit </h2>
            ...
            <div class="panel panel-default">
              ...
              <div class="panel-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <input type="text"  name="ctg_name" value="<?php if($ctg_name!="") { echo $ctg_name; } else { echo $row_state->ctg_name;} ?>" class="form-control" placeholder="ex: HDFC" required />
                  </div>
                </div>
                  ...
                  <div class="col-sm-9">
                    <select required class="form-control" name="ctg_status" >
                      ...
                      while ( $row_status = $result_status->fetch_object() ) {
                        ?>
                      <option <?php if($row_status->status_id==$row_state->ctg_status) { ?>selected<?php } ?> value="<?php echo $row_status->status_id?>" > <?php echo $row_status->status_name?> </option>
                      <?php } ?>
                    </select>
                  </div>
                </div>
              ...
              <div class="panel-footer">
                <div class="row">
                  <div class="col-sm-9 col-sm-offset-3">
                    ...
                    <input type="reset" style="background:#FFFFFF" class="btn btn-default" value="Cancel" onClick="location.href='category_view.php'">
                  </div>
                </div>
              </div>
            </div>
