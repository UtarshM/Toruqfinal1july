...
if ( isset( $_POST[ "submit" ] ) ) {
  ...
  $query_cmp_dup = "SELECT * FROM company_detail where cmp_id!='" . $cmp_id . "' and cmp_name='" . $cmp_name . "' and cmp_status='1'";
  ...
  if ( $total_records_dup >= 1 ) {
    ...
    $sql_company_updt = "UPDATE company_detail SET cmp_name='" . $cmp_name . "', updated_by='" . $updated_by . "', cmp_status='" . $cmp_status . "',  cmp_updated='" . $cmp_updated . "' WHERE cmp_id=" . $cmp_id;
    if ( $con->query( $sql_company_updt ) === TRUE ) {
      header( 'Location: company_view.php?flag=2' );
    } else {
      header( 'Location: company_edit.php?cmp_id=' . $cmp_id );
    }
  }
}
$query_state_detail = "SELECT * FROM company_detail ld where ld.cmp_id=" . $cmp_id;
...
<title>Company Edit -<?php echo " ".$project_title." "; ?></title>
...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-pen"></i> Company Edit </h2>
            ...
            <div class="panel panel-default">
              ...
              <div class="panel-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <input type="text"  name="cmp_name" value="<?php if($cmp_name!="") { echo $cmp_name; } else { echo $row_state->cmp_name;} ?>" class="form-control" placeholder="ex: HDFC" required />
                  </div>
                </div>
                  ...
                  <div class="col-sm-9">
                    <select required class="form-control" name="cmp_status" >
                      ...
                      while ( $row_status = $result_status->fetch_object() ) {
                        ?>
                      <option <?php if($row_status->status_id==$row_state->cmp_status) { ?>selected<?php } ?> value="<?php echo $row_status->status_id?>" > <?php echo $row_status->status_name?> </option>
                      <?php } ?>
                    </select>
                  </div>
                </div>
              ...
              <div class="panel-footer">
                <div class="row">
                  <div class="col-sm-9 col-sm-offset-3">
                    ...
                    <input type="reset" style="background:#FFFFFF" class="btn btn-default" value="Cancel" onClick="location.href='company_view.php'">
                  </div>
                </div>
              </div>
            </div>
