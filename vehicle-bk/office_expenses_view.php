...
// and Date(td.oexp_date) >= '$to_da' and Date(td.oexp_date) <= '$la_da'
// and Date(td.oexp_date) >= '$to_da' and Date(td.oexp_date) <= '$la_da'
// and Date(td.oexp_date) >= '$to_da' and Date(td.oexp_date) <= '$la_da'
// and Date(td.oexp_date) >= '$to_da' and Date(td.oexp_date) <= '$la_da'
...
// echo $oexp_incexp; exit;
  ...
  $query_expe_tot = "SELECT * FROM office_expenses_detail td where  td.oexp_status=1 ".$aol_tp_qu." and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.oexp_status IN (1,2) and Date(td.oexp_date) >= '$strdate' and Date(td.oexp_date) <= '$enddate' ORDER BY td.oexp_id";
  $result_tot = $con->query( $query_expe_tot );
  $total_records = $result_tot->num_rows;

  $query_expe = "SELECT * FROM office_expenses_detail td  where td.oexp_status=1 ".$aol_tp_qu." and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.oexp_status IN (1,2) and Date(td.oexp_date) >= '$strdate' and Date(td.oexp_date) <= '$enddate'  ORDER BY td.oexp_id";
...
$query_expe_tot = "SELECT * FROM office_expenses_detail td where  td.oexp_incexp IN(1,2) and td.oexp_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.oexp_status IN (1,2)  ORDER BY td.oexp_id";
$result_tot = $con->query( $query_expe_tot );
$total_records = $result_tot->num_rows;
// echo $total_records; exit;

$query_expe = "SELECT * FROM office_expenses_detail td  where td.oexp_incexp IN(1,2) and td.oexp_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.oexp_status IN (1,2)   ORDER BY td.oexp_id";

  $query_income = "SELECT SUM(oexp_amount) as total_income FROM office_expenses_detail td where td.oexp_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.oexp_incexp=1 ";
	$result_income = $con->query( $query_income );
	$row_income = $result_income->fetch_object();
	$final_income = $row_income->total_income;

	$query_expense = "SELECT SUM(oexp_amount) as total_expense FROM office_expenses_detail td where td.oexp_status=1 and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.oexp_incexp=2 ";
	$result_expense = $con->query( $query_expense );
	$row_expense = $result_expense->fetch_object();
	$final_expense = $row_expense->total_expense;
...
if ($_POST["submit_report"] != "") {
  ...
  $strdate = date("Y-m-d", strtotime($strdate));
  $enddate = date("Y-m-d", strtotime($enddate));
	header( 'Location: office_expenses_export.php?strdate=' . $strdate . '&enddate=' . $enddate.'&oexp_incexp=' . $oexp_incexp.'' );
}
...
<title>Office Expenses List   - <?php echo $meta_title; ?></title>
   ...
   function deleterec(id)
	{
		...
		{
			window.location="office_expenses_delete.php?oexp_id="+id;
		}
	} 
	</script> 
...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="pageheader">
      <h2><i class="fa fa-table"></i> Office Expenses List   </h2>
      <div class="breadcrumb-wrapper"> <span class="label">You are here:</span>
        <ol class="breadcrumb">
          <li><a style="color:#1C1B17;" href="#">Dashboard</a></li>
          <li class="active">Office Expenses List  </li>
        </ol>
      </div>
    </div>
      ...
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          if ( isset( $_GET[ 'flag' ] ) ) {
            ?>
          <?php if($_GET['flag']==1) {?>
          <p class="mb20" style="color:green">Office Expenses details added successfully.</p>
          <?php } else if($_GET['flag']==2) {?>
          <p class="mb20" style="color:green">Office Expenses details updated successfully.</p>
          <?php } else if($_GET['flag']==3) {?>
          <p class="mb20" style="color:green">Office Expenses details deleted successfully.</p>
          <?php } ?>
          <?php } ?>
          <div class="col-md-12" style="margin:0px 0px 20px 0px;">
            ...
            <div class="col-md-9">
              <form class="form-horizontal" method="post" action="">
                ...
                <select style="width: 100%;" class="form-control" name="oexp_incexp" >
                  <option value="" > Select  </option>
                  <option value="1" <?php if($_POST['oexp_incexp']==1) { ?>selected<?php } ?> >Income </option>
                  <option value="2" <?php if($_POST['oexp_incexp']==2) { ?>selected<?php } ?> >Expense </option>
                </select>
                </div>
            ...
            <table id="example" class="table table-success mb30 table-hover table-bordered display" style="color:#000;" >
              ...
              <tbody>
                ...
                while ( $row_state = $result->fetch_object() ) {
                  ?>
                <tr class="odd gradeX" style="background-color: <?php if($row_state->oexp_incexp==1) { ?>#E7F1E8<?php } else { ?>#FFD5D4<?php } ?> !important;">
                  <td ><?php echo $row_state->oexp_id;?></td>
                  <td ><?php echo $row_state->oexp_code_no;?></td>
                  <td ><?php
                  $datend = new DateTime( $row_state->oexp_date );
                  echo $datend->format( 'D, d-m-Y' );
                  ?></td>
                  <td><a title="Edit" style="color:green;" href="office_expenses_edit.php?oexp_id=<?php echo $row_state->oexp_id?>"><?php echo $row_state->oexp_description;?></a></td>
                  <td><?php echo "&#8377;".number_format($row_state->oexp_amount,2);?></td>
                  
                   
                  <td ><?php if($row_state->oexp_incexp==1) {  echo "Income"; } elseif($row_state->oexp_incexp==2) { echo "Expense"; } ?></td>
                  <td   width="17%"><code> <a title="Edit" style="color:green;" href="office_expenses_edit.php?oexp_id=<?php echo $row_state->oexp_id?>"><i class="fa fa-pen"></i></a> | <a title="Delete" style="color:red;" href="javascript:deleterec(<?php echo $row_state->oexp_id?>)"><i class="fa fa-trash"></i></a> <code></td>
                </tr>
                <?php }} ?>
              </tbody>
              <tfoot>
