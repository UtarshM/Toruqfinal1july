...
if($_POST["submit_search"]!=""){
  ...
  // echo $date_type; exit;
    ...
    if ( $_SESSION[ 'adm_type' ] == 0 ) { // Admin 
      // Check Module Rights
      $query_expe_tot = "SELECT * FROM taken_detail td,  status_detail st  where td.tkn_status !=3 and td.tkn_action IN (1,2) ".$dtws_qu." and st.status_id=td.tkn_status  and td.branch_id=".$_SESSION[ 'adm_branch' ]." ORDER BY td.tkn_id";
      $result_tot = $con->query( $query_expe_tot );
      $total_records = $result_tot->num_rows;

      $query_expe = "SELECT * FROM taken_detail td, status_detail st  where td.tkn_status !=3 and td.tkn_action IN (1,2) ".$dtws_qu." and st.status_id=td.tkn_status  and td.branch_id=".$_SESSION[ 'adm_branch' ]." ORDER BY td.tkn_id";
    } else {
      // Check Module Rights
      $query_expe_tot = "SELECT * FROM taken_detail td,  status_detail st  where td.tkn_status !=3 and td.tkn_action IN (1,2) ".$dtws_qu." and st.status_id=td.tkn_status  and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.tkn_adm_id=".$_SESSION[ 'adm_id' ]." ORDER BY td.tkn_id";
      $result_tot = $con->query( $query_expe_tot );
      $total_records = $result_tot->num_rows;

      $query_expe = "SELECT * FROM taken_detail td, status_detail st  where td.tkn_status !=3 and td.tkn_action IN (1,2) ".$dtws_qu." and st.status_id=td.tkn_status  and td.branch_id=".$_SESSION[ 'adm_branch' ]." and td.tkn_adm_id=".$_SESSION[ 'adm_id' ]." ORDER BY td.tkn_id";
    }
  
  } else {

    if($date_type==1){
      // $dtws_qu = "and Date(td.tkn_insurance_date) >= '$to_da' and Date(td.tkn_insurance_date) <= '$la_da'";
      $fl_nam = "Insurance";
    } else if($date_type==2){
      // $dtws_qu = "and Date(td.tkn_cf_date) >= '$to_da' and Date(td.tkn_cf_date) <= '$la_da'";
      $fl_nam = "CF";
    } else if($date_type==3){
      // $dtws_qu = "and Date(td.tkn_reg_date) >= '$to_da' and Date(td.tkn_reg_date) <= '$la_da'";
      $fl_nam = "Registration";
    } else if($date_type==4){
      // $dtws_qu = "and Date(td.tkn_permit_date) >= '$to_da' and Date(td.tkn_permit_date) <= '$la_da'";
      $fl_nam = "Permit";
    } else if($date_type==5){
      // $dtws_qu = "and Date(td.tkn_nat_permit_date) >= '$to_da' and Date(td.tkn_nat_permit_date) <= '$la_da'";
      $fl_nam = "National Permit";
    } else if($date_type==6){
      // $dtws_qu = "and Date(td.tkn_tax_date) >= '$to_da' and Date(td.tkn_tax_date) <= '$la_da'";
      $fl_nam = "Tax";
    } 
  ...
  if ($_POST["submit_report"] != "") {
    ...
    $enddate = date("Y-m-d", strtotime($enddate));
    $date_type = $_REQUEST['date_type'];
    header( 'Location: taken_export.php?strdate='.$strdate.'&enddate='.$enddate.'&date_type='.$date_type.'' );
  }
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Taken List   - <?php echo $meta_title; ?></title>
   ...
   function deleterec(id)
	{
		...
		{
			window.location="taken_delete.php?tkn_id="+id;
		}
	} 
...
<section>
  ...
  <div class="mainpanel">
    ...
    <div class="contentpanel">
      <div class="panel panel-default">
        <div class="panel-body">
          ...
          if ( isset( $_GET[ 'flag' ] ) ) {
            ?>
          <?php if($_GET['flag']==1) {?>
          <p class="mb20" style="color:green">Taken details added successfully.</p>
          <?php } else if($_GET['flag']==2) {?>
          <p class="mb20" style="color:green">Taken details updated successfully.</p>
          <?php } else if($_GET['flag']==3) {?>
          <p class="mb20" style="color:green">Taken details deleted successfully.</p>
          <?php } else if($_GET['flag']==4) {?>
          <p class="mb20" style="color:green">Taken details assign action applied successfully.</p>
          <?php } else if($_GET['flag']==6) {?>
          <p class="mb20" style="color:red">Taken details deleted successfully.</p>
      ...
      <form method="POST" action="taken_excelUpload.php" enctype="multipart/form-data">
    <div class="form-group col-sm-6">
      <h5>Upload Taken Data by excel file <span class="asterisk">*</span></h5>
      <input type="file" name="file" required class="form-control">
     </div>
    <div class="form-group col-sm-6">
      <h5><a href="img/Taken-Sample.xls">Download sample xls file</a></h5> 
      <button type="submit" name="submit_taken_excel"  class="btn">Upload Excel</button>
    </div>
...
<!-- Import Excel For MCQ -->
<form method="post" action="taken_action_porcess.php">
<?php if ( $_SESSION[ 'adm_type' ] == 0 ) { // Admin  ?>
          <div class="form-group col-sm-12 col-md-12 col-lg-12" style="margin-top: 25px;">
          <select name="tkn_adm_id" required class="form-control" style="width: 250px;">
            ...
            <table id="example" class="table table-success mb30 table-hover table-bordered display" style="color:#000;" >
              ...
              <tbody>
                ...
                while ( $row_state = $result->fetch_object() ) {
                  ?>
                <tr class="odd gradeX">
                <?php if ( $_SESSION[ 'adm_type' ] == 0 ) { // Admin  ?><td><input type="checkbox" name="selected_tkn_id[]" value="<?php echo $row_state->tkn_id; ?>" onclick="handleCheckboxClick()"></td><?php } ?>

                  <td ><?php echo $row_state->tkn_series;?></td>
                   <td >
                  <?php 
                   if($date_type==1){
                    $datend = new DateTime( $row_state->tkn_insurance_date );
                    echo $datend->format( 'd-m-Y' );
                  } else if($date_type==2){
                  $datend = new DateTime( $row_state->tkn_cf_date );
                    echo $datend->format( 'd-m-Y' );
                  } else if($date_type==3){
                    $datend = new DateTime( $row_state->tkn_reg_date );
                      echo $datend->format( 'd-m-Y' );
                    } else if($date_type==4){
                      $datend = new DateTime( $row_state->tkn_permit_date );
                        echo $datend->format( 'd-m-Y' );
                      } else if($date_type==5){
                        $datend = new DateTime( $row_state->tkn_nat_permit_date );
                          echo $datend->format( 'd-m-Y' );
                        } else if($date_type==6){
                          $datend = new DateTime( $row_state->tkn_tax_date );
                            echo $datend->format( 'd-m-Y' );
                          } else {
                            $datend = new DateTime( $row_state->tkn_date );
                          echo $datend->format( 'd-m-Y' );
                          }
                  ...
                  ?>
                    <?php 
                  ?></td>
                  <td><?php echo $row_state->tkn_reg_no;?></td>
                  <td><a title="Edit" style="color:green;" href="taken_edit.php?tkn_id=<?php echo $row_state->tkn_id?>"><?php echo $row_state->tkn_name;?></a></td>
                  <td><?php echo $row_state->tkn_contact;?></td>
                   <td><?php echo $row_state->tkn_vmodel;?></td>
                  <td><?php
                  if($row_state->tkn_adm_id!=0) {
                    $query_asign_detail = "SELECT * FROM admin_login ld where ld.adm_id=".$row_state->tkn_adm_id;
                  ...
                  <td style="color: <?php if($row_state->tkn_action==1) { ?>#82c21f<?php } elseif($row_state->tkn_action==2) { ?>red;<?php } ?>">
                    <?php if($row_state->tkn_action==1) {
                      echo "Pending";
                    } elseif($row_state->tkn_action==2) {
                      echo "Rejected - ";
                      ...
                      echo $rejres_name;
                    } elseif($row_state->tkn_action==3) {
                      echo "Completed";
                    }
                    ?> 
                    
                  <?php // echo $row_state->status_name;?></td>
                  <td  width="17%"><code> <a  title="Follow Up"style="color:#1C1B17; cursor:pointer; " onClick="return taken_followup(<?php echo $row_state->tkn_id?>);" ><i class="fa fa-comment"></i></a> | <a target="_blank" style="color:#333;" title="Documents" href="taken_documents.php?tkn_id=<?php echo $row_state->tkn_id?>"><i class="fa fa-image"></i></a> | <a title="Edit" style="color:green;" href="taken_edit.php?tkn_id=<?php echo $row_state->tkn_id?>"><i class="fa fa-pen"></i></a> <?php if($_SESSION['adm_type']==0) { ?>| <a title="Delete" style="color:red;" href="javascript:deleterec(<?php echo $row_state->tkn_id?>)"><i class="fa fa-trash"></i></a><?php } ?> <code></td>
                </tr>
                <?php }} ?>
              </tbody>
              <tfoot>
...
<script>
  ...
  function handleCheckboxClick() {
    var checkboxes = document.getElementsByName('selected_tkn_id[]');
