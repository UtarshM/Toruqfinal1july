...
if ( in_array( "3", $md_right ) ) {
  ...
  if ( $_SESSION[ 'adm_type' ] == 0 ) { // Admin 
    ...
    $query_followup = "SELECT * FROM tkn_flp_detail td, admin_login al, taken_detail fd  where td.tkn_id=fd.tkn_id and td.branch_id=".$_SESSION[ 'adm_branch' ]." and al.adm_id=fd.tkn_adm_id and Date(td.tknflp_reminder_date) = '$tod_date'  ORDER BY td.tknflp_id DESC";
    ...
    $query_followup = "SELECT * FROM tkn_flp_detail td, admin_login al, taken_detail fd  where td.tkn_id=fd.tkn_id and td.branch_id=".$_SESSION[ 'adm_branch' ]." and fd.tkn_adm_id=".$_SESSION[ 'adm_id' ]." and al.adm_id=fd.tkn_adm_id and Date(td.tknflp_reminder_date) = '$tod_date'  ORDER BY td.tknflp_id DESC";
    $result_flow_tot = $con->query( $query_followup );
    $total_records_flow = $result_flow_tot->num_rows;
