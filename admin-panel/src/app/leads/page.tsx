...
export default function LeadsPage() {
  ...
  const getLeadColumnValue = (lead: any, colKey: string): string => {
    ...
    if (colKey === 'assignedTo') return lead.assignee?.fullName || 'Unallotted'
    return '—'
  }
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">List Management</h1>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {isAdmin && selectedAssignedIds.length > 0 && (
            <button 
              ...
              title="De-allot selected assigned leads and return them to the unallotted pool"
            >
              <UserX size={14} />
              {isDeassigning ? 'De-allotting...' : `De-allot (${selectedAssignedIds.length})`}
            </button>
          )}
          {/* Bulk Delete Button */}
          {selectedIds.size > 0 && (
          ...
          {/* Import List & New Entry → Visible for Admins / Managers */}
          {!isExecutive && (
            <>
              <button 
            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                {[
                  ...
                  { key: 'assignedTo', label: 'Allotment', type: 'select' },
                    ...
                    <td className="px-3 py-3 text-xs text-slate-600 max-w-[140px]" onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-1.5">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold truncate ${
                          ...
                          {lead.assignee?.fullName || 'Unallotted'}
                        </span>
                        {isAdmin && lead.assignedTo && (
                          <button
                            ...
                            title="De-allot this lead (return to unallotted pool)"
                    ...
                    <div className="space-y-6">
                      {isEditing ? (
                        ...
                        <>
                          ...
                          <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4">
                            <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Allotment to auto advisor</label>
                            {isExecutive ? (
                              <div className="bg-white border border-slate-200 rounded-xl p-3 text-xs font-bold text-slate-800 flex items-center justify-between shadow-2xs">
                                <span>{detailedLead.assignee?.fullName || 'Unallotted'}</span>
                              ...
                              <>
                                <select 
                                  ...
                                  className="w-full bg-white border border-slate-200 rounded-xl p-2.5 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-slate-100"
                                >
                                  <option value="unassigned">Unallotted (Leave Open)</option>
                                  {employees.map(emp => (
